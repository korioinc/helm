...
# Complete custom profile: add Go and Rust to the same final prefix.
case "$ENV_PLATFORM" in
  linux/amd64)
    go_arch=amd64; go_sha=031f088e5d955bab8657ede27ad4e3bc5b7c1ba281f05f245bcc304f327c987a
    rust_target=x86_64-unknown-linux-gnu; rust_sha=b4cdbc7cc6b0ee0a2666b1872769fdb2ad8393b28b63952f6493b4b400e4832b ;;
  linux/arm64)
    go_arch=arm64; go_sha=a290581cfe4fe28ddd737dde3095f3dbeb7f2e4065cab4eae44dfc53b760c2f7
    rust_target=aarch64-unknown-linux-gnu; rust_sha=2f2496c70bd336a66a4c8baf2d303ba161f3552f192444c3639ba903c7c1e2c5 ;;
esac
curl -fsSL "https://go.dev/dl/go1.26.1.linux-${go_arch}.tar.gz" -o "$task_tmp/go.tar.gz"
printf '%s  %s\n' "$go_sha" "$task_tmp/go.tar.gz" | sha256sum -c -
tar -xzf "$task_tmp/go.tar.gz" -C "$ENV_ROOT"
curl -fsSL "https://static.rust-lang.org/dist/rust-1.97.1-${rust_target}.tar.gz" -o "$task_tmp/rust.tar.gz"
printf '%s  %s\n' "$rust_sha" "$task_tmp/rust.tar.gz" | sha256sum -c -
tar -xzf "$task_tmp/rust.tar.gz" -C "$task_tmp"
"$task_tmp/rust-1.97.1-${rust_target}/install.sh" --prefix="$ENV_ROOT/rust" --without=rust-docs --disable-ldconfig
node - <<'NODE'
const fs = require('node:fs'), p = process.env.ENV_MANIFEST_FILE;
const manifest = JSON.parse(fs.readFileSync(p));
manifest.binDirs.push('go/bin','rust/bin');
Object.assign(manifest.env,{GOPATH:'${HOME}/go',GOCACHE:'${HOME}/.cache/go-build',CARGO_HOME:'${HOME}/.cargo'});
manifest.checks.push({argv:['go/bin/go','version'],timeoutSeconds:10},{argv:['rust/bin/rustc','--version'],timeoutSeconds:10});
