...
# OS tools are supplied by the operator image, not an init-time apt install.
php -r 'if (!extension_loaded("curl")) { exit(1); }'
python3 -m venv --copies "$ENV_ROOT/venv"
"$ENV_ROOT/venv/bin/pip" --version
cat > "$ENV_ROOT/bin/php-curl-check" <<'PHP'
#!/bin/bash
exec php -r 'exit(extension_loaded("curl") ? 0 : 1);'
PHP
chmod 0555 "$ENV_ROOT/bin/php-curl-check"
node - <<'NODE'
const fs = require('node:fs'), p = process.env.ENV_MANIFEST_FILE;
const manifest = JSON.parse(fs.readFileSync(p));
manifest.binDirs.push('venv/bin');
manifest.env.PIP_CACHE_DIR='${HOME}/.cache/pip';
manifest.env.PYTHONPYCACHEPREFIX='${HOME}/.cache/python';
manifest.checks.push({argv:['bin/php-curl-check'],timeoutSeconds:10},{argv:['venv/bin/pip','--version'],timeoutSeconds:10});
