...
Chart source lives in `main`. Increase the chart's `version` in `Chart.yaml` when publishing a change, then push to `main`. The release workflow verifies the chart, publishes its package to GitHub Releases, updates `index.yaml` and site metadata on `gh-pages`, and builds GitHub Pages.
